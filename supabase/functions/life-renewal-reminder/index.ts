...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch records requiring renewal from "life_insurance" table
    const { data: rows, error: fetchErr } = await supabase
      .from("life_insurance")
      .select("*");

    if (fetchErr) {
      console.error("Error fetching records from life_insurance:", fetchErr);
      return new Response(
    ...
    if (rows) {
      ...
      for (const row of rows) {
        ...
        const recordId = `life_insurance_${row.id}`;
        ...
        // Send WhatsApp reminder message using "life_renewal_reminder" template
        let status = "Failed";
        ...
        if (!WHATSAPP_ACCESS_TOKEN || !PHONE_NUMBER_ID) {
          ...
          try {
            const templateName = reqBody?.template_name || "life_renewal_reminder";
            ...
            if (documentUrl) {
              let filename = `Life_Insurance_${policyNo.replace(/[^a-zA-Z0-9.-]/g, "_")}`;
        ...
        await supabase.from("renewal_notification_history").insert([
          {
            renewal_id: recordId,
            document_name: `Life Insurance (${policyNo})`,
    ...
    const summary = {
      template: "life_renewal_reminder",
