...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch records requiring renewal from "health_insurance" table
    const { data: rows, error: fetchErr } = await supabase
      .from("health_insurance")
      .select("*");

    if (fetchErr) {
      console.error("Error fetching records from health_insurance:", fetchErr);
      return new Response(
    ...
    if (rows) {
      ...
      for (const row of rows) {
        ...
        const recordId = `health_insurance_${row.id}`;
        const concernPerson = row.concern_person_name || "Concerned Person";
        const companyName = row.company_name || "";
        const policyHolder = row.policy_holder || "";
        const policyNo = row.policy_no || "";
        const insuranceAgent = row.insurance_agent || "N/A";
        const documentUrl = row.document_url && typeof row.document_url === "string" && row.document_url.trim() ? row.document_url.trim() : null;
        ...
        // Send WhatsApp reminder message using "health_renewal_reminder" template
        let status = "Failed";
        ...
        if (!WHATSAPP_ACCESS_TOKEN || !PHONE_NUMBER_ID) {
          ...
          try {
            const templateName = reqBody?.template_name || "health_renewal_reminder";
            ...
            const bodyParameters = [
              ...
              { type: "text", text: String(policyHolder) },
              { type: "text", text: String(policyNo) },
              { type: "text", text: String(insuranceAgent) },
              { type: "text", text: String(formattedDateParam) },
            ];

            // Build Header Component if document_url is present in DB
            let headerComponent: any = null;
            if (documentUrl) {
              let filename = `Health_Insurance_${policyNo.replace(/[^a-zA-Z0-9.-]/g, "_")}`;
        ...
        await supabase.from("renewal_notification_history").insert([
          {
            renewal_id: recordId,
            document_name: `Health Insurance (${policyNo})`,
    ...
    const summary = {
      template: "health_renewal_reminder",
