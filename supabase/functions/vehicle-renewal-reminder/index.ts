...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch records requiring renewal from "vehicle_insurance" table
    const { data: rows, error: fetchErr } = await supabase
      .from("vehicle_insurance")
      .select("*");

    if (fetchErr) {
      console.error("Error fetching records from vehicle_insurance:", fetchErr);
      return new Response(
    ...
    if (rows) {
      ...
      for (const row of rows) {
        ...
        const recordId = `vehicle_insurance_${row.id}`;
        const concernPerson = row.concern_person_name || "Concerned Person";
        const companyName = row.company_name || "";
        const registrationNo = row.registration_no || "";
        const insuranceAgent = row.insurance_agent || "N/A";
        const fileUrl = row.file_url && typeof row.file_url === "string" && row.file_url.trim() ? row.file_url.trim() : null;
        ...
        // Send WhatsApp reminder message using "vehicle_renewal_reminder" template
        let status = "Failed";
        ...
        if (!WHATSAPP_ACCESS_TOKEN || !PHONE_NUMBER_ID) {
          ...
          try {
            const templateName = reqBody?.template_name || "vehicle_renewal_reminder";
            ...
            const bodyParameters = [
              ...
              { type: "text", text: String(registrationNo) },
              { type: "text", text: String(insuranceAgent) },
              { type: "text", text: String(formattedDateParam) },
            ];

            // Build Header Component if file_url is present in DB
            let headerComponent: any = null;
            if (fileUrl) {
              let filename = `Vehicle_Insurance_${registrationNo.replace(/[^a-zA-Z0-9.-]/g, "_")}`;
              if (fileUrl.includes(".")) {
        ...
        await supabase.from("renewal_notification_history").insert([
          {
            renewal_id: recordId,
            document_name: `Vehicle Insurance (${registrationNo})`,
    ...
    const summary = {
      template: "vehicle_renewal_reminder",
