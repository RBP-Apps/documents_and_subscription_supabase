...
const LifeRenewal = () => {
  ...
  useEffect(() => {
    setTitle('Life Insurance Renewal');
  }, [setTitle]);

  const [activeTab, setActiveTab] = useState<'pending' | 'history' | 'reminders'>('pending');
  const [searchTerm, setSearchTerm] = useState('');
  const [pendingData, setPendingData] = useState<LifeInsurance[]>([]);
  ...
  const [selectedItem, setSelectedItem] = useState<LifeInsurance | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      console.error('Error loading life renewals:', err);
  ...
  const handleOpenRenewal = (item: LifeInsurance) => {
  ...
  const handleSaveRenewal = async (e: React.FormEvent) => {
    ...
    try {
      let uploadedFileUrl = selectedItem.document_url || null;
      ...
      // 2. Update the life insurance record
      const mainUpdates = {
      ...
      const historyLog = {
        ...
        old_image: selectedItem.document_url || null,
        need_renewal: againRenewal,
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Life Insurance Renewals</h1>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-fade-in">
            ...
            <div className="flex justify-between items-center px-5 py-4 border-b border-gray-100 bg-gray-50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-800">Renew Life Insurance</h3>
              </div>
              <button
            ...
            <form onSubmit={handleSaveRenewal} className="p-5 space-y-4">
              <div>
                ...
                <div className="mt-1.5 p-3 rounded-lg bg-gray-50 text-xs space-y-1 text-gray-700">
                  ...
                  <div className="flex justify-between">
                    <span className="font-medium text-gray-500">Policy No:</span>
                    <span className="font-semibold text-gray-900 font-mono">{selectedItem.policy_no}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium text-gray-500">Holder Name:</span>
                    <span className="font-semibold text-gray-900">{selectedItem.policy_holder}</span>
                  </div>
                  <div className="flex justify-between">
