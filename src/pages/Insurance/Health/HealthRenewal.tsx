...
const HealthRenewal = () => {
  ...
  useEffect(() => {
    setTitle('Health Insurance Renewal');
  }, [setTitle]);

  const [activeTab, setActiveTab] = useState<'pending' | 'history' | 'reminders'>('pending');
  const [searchTerm, setSearchTerm] = useState('');
  const [pendingData, setPendingData] = useState<HealthInsurance[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      console.error('Error loading health renewals:', err);
  ...
  const handleOpenRenewal = (item: HealthInsurance) => {
  ...
  const handleSaveRenewal = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Update the health insurance record
      const mainUpdates = {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Health Insurance Renewals</h1>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-fade-in">
            ...
            <div className="flex justify-between items-center px-5 py-4 border-b border-gray-100 bg-gray-50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-800">Renew Health Insurance</h3>
              </div>
              <button
