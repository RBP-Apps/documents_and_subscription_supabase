...
const WorkManCompensation = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<WorkmanCompensation | null>(null);
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr className="text-xs uppercase text-gray-600 font-semibold tracking-wider">
                  ...
                  <th className="px-6 py-4 text-right">FINAL PREMIUM</th>
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (fileUpload) {
        ...
        const filePath = `general/employee compensation/${Date.now()}_${cleanFileName}`;
...
const ShareDropdown = ({
  ...
  item: WorkmanCompensation;
  onShare: (type: 'email' | 'whatsapp' | 'both', item: WorkmanCompensation) => void;
}) => {
...
interface ShareCompensationModalProps {
  isOpen: boolean;
  onClose: () => void;
  type: 'email' | 'whatsapp' | 'both' | null;
  item: WorkmanCompensation | null;
}

const ShareCompensationModal: React.FC<ShareCompensationModalProps> = ({
  isOpen,
  onClose,
  type,
  item,
  ...
  useEffect(() => {
    if (isOpen && item) {
      ...
      setEmail('');
      setWhatsapp('');
      setSubject(`Sharing Workmen's Compensation: ${item.company_name} (${item.policy_no})`);
      setMessage(`Please find the link for the shared Workmen's Compensation document: ${item.company_name} (${item.policy_no}).`);
      setEmailSent(false);
      setIsSending(false);
    }
  }, [isOpen, item]);
  ...
  const handleShareWhatsApp = async (): Promise<boolean> => {
    ...
    const docName = `${item.company_name} Workmen's Compensation (${item.policy_no})`;
    ...
    try {
      ...
      const { error } = await supabase.from('Shared_Documents').insert([{
        ...
        document_type: "Workmen's Compensation",
        category: 'Insurance',
        serial_no: item.serial_no || `EC-${String(item.id).padStart(3, '0')}`,
        image: docLink !== 'N/A' ? docLink : null,
        share_method: 'WhatsApp',
        number: whatsapp,
        source_sheet: 'Workman Compensation',
      }]);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-lg text-sm text-indigo-700 font-medium truncate">
              📄 {item.company_name} Workmen's Compensation ({item.policy_no})
            </div>
          </div>
