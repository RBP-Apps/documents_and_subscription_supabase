      ...
      if (masterData.email_renewal_details && masterData.email_renewal_details.length > 0) {
        ...
        setServices(
          sorted.map((item) => ({
            ...
            need_renewal: item.need_renewal || false,
            renewal_date: item.renewal_date || '',
            concern_person_name: item.concern_person_name || '',
            concern_person_mobile: item.concern_person_mobile || '',
            concern_person_department: item.concern_person_department || '',
          }))
        );
      } else {
        setServices([
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const detailsToInsert = services.map((s) => ({
        ...
        need_renewal: s.need_renewal || false,
