...
const PannelTestReports = () => {
  const { setTitle } = useHeaderStore();
  const [data, setData] = useState<PannelReport[]>([]);
  ...
  const [selectedReport, setSelectedReport] = useState<PannelReport | null>(null);

  const [shareOpen, setShareOpen] = useState(false);
  const [shareType, setShareType] = useState<'email' | 'whatsapp' | 'both' | null>(null);
  const [selectedItem, setSelectedItem] = useState<PannelReport | null>(null);

  useEffect(() => {
    setTitle('Pannel Test Reports');
    fetchReports();
  }, [setTitle]);
  ...
  const fetchReports = async () => {
    try {
      ...
      const local = localStorage.getItem('pannel_test_reports');
  ...
  const saveToLocalStorage = (newData: PannelReport[]) => {
    localStorage.setItem('pannel_test_reports', JSON.stringify(newData));
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const { error } = await supabase.from('pannel_test_reports').delete().eq('id', id);
      if (error) throw error;
  ...
  const handleViewFile = (item: PannelReport) => {
  ...
  const handleEdit = (item: PannelReport) => {
  ...
  const filteredData = data.filter((item) => {
    const matchesSearch =
      ...
      item.panel_type?.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          <div className="p-4 border-b border-gray-200">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                ...
                <p className="text-gray-500 text-xs mt-1">Manage all Pannel Test Report records</p>
              </div>
            ...
            <table className="w-full text-left">
              <thead className="sticky top-0 z-20 bg-gray-50 border-b border-gray-200">
                <tr className="text-xs uppercase text-gray-600 font-semibold tracking-wider">
                  ...
                  <th className="px-6 py-4">PANNEL TYPE</th>
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-indigo-50/30 transition">
                    ...
                    <td className="px-6 py-4 text-gray-600">{item.panel_type}</td>
                  ...
                  <tr>
                    <td colSpan={12} className="text-center py-12 text-gray-500">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p>No Pannel Test Report Records Found</p>
                      </div>
                    </td>
                  </tr>
                )}
...
const ShareDropdown = ({
  ...
  item: PannelReport;
  onShare: (type: 'email' | 'whatsapp' | 'both', item: PannelReport) => void;
}) => {
...
// Add Modal Component
interface AddPannelModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  currentData: PannelReport[];
  saveLocal: (data: PannelReport[]) => void;
}

const AddPannelModal: React.FC<AddPannelModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  currentData,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!state || !date || !department || !scheme || !companyName || !panelType || !rating) {
    ...
    try {
      if (file) {
        ...
        const filePath = `test_reports/pannel/${Date.now()}_${cleanFileName}`;
      ...
      const serialNo = `PNL-${inserted.id}`;
      await supabase.from('pannel_test_reports').update({ serial_no: serialNo }).eq('id', inserted.id);

      toast.success('Pannel Test Report added successfully');
      onSuccess();
      onClose();
    } catch (error: any) {
      ...
      const newReport: PannelReport = {
        id: newId,
        serial_no: `PNL-${newId.toString().slice(-4)}`,
        state,
        date,
        department,
        scheme,
      ...
      saveLocal(updated);
      toast.success('Pannel Test Report added successfully (Local)');
      onSuccess();
      onClose();
    } finally {
      ...
      <div className="relative my-4 w-full max-w-2xl bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-blue-50">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-bold text-gray-800">Add Pannel Test Report</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-2.5 text-sm border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                placeholder="e.g. Solar Drinking Water"
              />
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Pannel Type *</label>
              <input
                type="text"
                required
                value={panelType}
                ...
                className="w-full p-2.5 text-sm border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                placeholder="e.g. VFD Control Panel"
              />
            </div>
            <div>
...
interface EditPannelModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  reportData: PannelReport;
  currentData: PannelReport[];
  saveLocal: (data: PannelReport[]) => void;
}

const EditPannelModal: React.FC<EditPannelModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  reportData,
  ...
  const [panelType, setPanelType] = useState(reportData.panel_type);
  const [rating, setRating] = useState(reportData.rating);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Pannel Test Report updated successfully');
      onSuccess();
      onClose();
    } catch (error: any) {
      ...
      saveLocal(updated);
      toast.success('Pannel Test Report updated successfully (Local)');
      onSuccess();
      onClose();
    } finally {
      setIsSubmitting(false);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 bg-black/40 backdrop-blur-sm overflow-y-auto">
      <div className="relative my-4 w-full max-w-2xl bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-blue-50">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-bold text-gray-800">Edit Pannel Test Report</h2>
          </div>
          <button onClick={onClose} disabled={isSubmitting} className="p-1.5 text-gray-400 hover:text-gray-600 rounded-full transition">
            <X size={20} />
          </button>
...
interface SharePannelModalProps {
  isOpen: boolean;
  onClose: () => void;
  type: 'email' | 'whatsapp' | 'both' | null;
  item: PannelReport | null;
}

const SharePannelModal: React.FC<SharePannelModalProps> = ({
  isOpen,
  onClose,
  type,
  item,
  ...
  useEffect(() => {
    if (isOpen && item) {
      ...
      setEmail('');
      setWhatsapp('');
      setSubject(`Sharing Pannel Test Report: ${item.company_name} - ${item.scheme}`);
      setMessage(`Please find the link for the shared Pannel Test Report document: ${item.company_name} (${item.scheme}, ${item.state}).`);
      setEmailSent(false);
      setIsSending(false);
    }
  }, [isOpen, item]);
  ...
  const handleShareWhatsApp = async (): Promise<boolean> => {
    ...
    const docName = `Pannel Test Report (${item.company_name} - ${item.scheme})`;
      ...
      const { error } = await supabase.from('Shared_Documents').insert([
        {
          ...
          document_type: 'Pannel Test Report',
          category: 'Project Documents',
          serial_no: item.serial_no,
          image: docLink !== 'N/A' ? docLink : null,
          share_method: 'WhatsApp',
          number: whatsapp,
          source_sheet: 'Pannel Test Reports',
        },
      ]);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-lg text-sm text-indigo-700 font-medium truncate">
              📄 Pannel Report: {item.company_name} ({item.scheme})
            </div>
          </div>
...
export default PannelTestReports;
